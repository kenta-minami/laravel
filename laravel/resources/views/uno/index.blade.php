...
@if($next!==1)
  @if($gameend!==1)
    ...
    <!-- @if($turn%2===0) -->
      @foreach ($playCards as $key => $playCard)
        <a href="{{route('uno.next',['id' => $playCards[$key]['display'],'no'=>$key, 'status'=>'exit'],false)}}">{{ $playCard['display'] }}</a>
      @endforeach
    <!-- @endif -->
  ...
  <?php if(!$nimaime && $wildflag!==1):?>
      ...
      <?php else:?>
          ...
          <?php echo "500点を超えたので試合を終了します。resetを押してください。<br>";?>
      <?php endif;?>
  <?php elseif($nimaime):?>
      <?php echo "<br>"; ?>
      ・・・出しますか？<br>
  ...
  ・・・<a href="{{route('uno.index',['no'=>'rule'])}}">☆ルール</a><br><br><br>
  <?php if($Exhibit===1){$turn--; }?>
  <br>
@else
  <a href="{{route('uno.next')}}">{{$maeCard}}</a><br>
@endif
